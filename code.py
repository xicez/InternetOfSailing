...
# i2c = busio.I2C(scl=board.GP3, sda=board.GP2)  # configure GP2 and GP3 for scl and sda respectively
...
status_readout = label.Label(terminalio.FONT, text="Status: Logging", color=0xFFFFFF, x=8, y=8)
heel_angle_readout = label.Label(terminalio.FONT, text="Heel:     ", color=0xFFFFFF, x=8, y=22)
gps_speed_readout = label.Label(terminalio.FONT, text="Speed:    ", color=0xFFFFFF, x=8, y=34)


main_group.append(status_readout)
main_group.append(heel_angle_readout)
main_group.append(gps_speed_readout)
...
HORIZONTAL_BAR_X_ORIGIN = 16
HORIZONTAL_BAR_Y_ORIGIN = 48
HORIZONTAL_BAR_WIDTH = 48
...
while True:
    for val in fake__accel_data:
        if val[0] >= 0:
            ...
            heel_angle_readout.text = f"Heel: {round(val[0],2)}"
            gps_speed_readout.text = f"Speed: {round(4 + random.uniform(-0.5, 1.5),1)} kts"
        if val[0] < 0:
